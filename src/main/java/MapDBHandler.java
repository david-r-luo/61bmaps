...
public class MapDBHandler extends DefaultHandler {
    ...
    public static final HashMap<Long, Node> NODESET = new HashMap<>();
    public static final HashMap<String, LinkedList<Node>> NAMETONODELIST = new HashMap<>();
    public static final LinkedList<String> NAMELIST = new LinkedList<>();
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            NODESET.put(Long.parseLong(attributes.getValue("id")), currNode);
        } else if (qName.equals("way")) {
            activeState = "way";
        } else if (activeState.equals("node") && qName.equals("tag")
                && attributes.getValue("k").equals("name")) {
            currNode.name = attributes.getValue("v");
            LinkedList ll = NAMETONODELIST.get(currNode.name);
            if (ll == null) {
                ll = new LinkedList();
            }
            ll.add(currNode);
            if (!NAMELIST.contains(currNode.name)) {
                NAMELIST.add(currNode.name);

            }
            NAMETONODELIST.put(currNode.name, ll);
        ...
        if (qName.equals("way")) {
            if (toAdd.get(toAdd.size() - 1) < 0) {
                for (int i = 0; i < toAdd.size() - 2; i++) {
                    int j = i + 1;
                    Node n1 = NODESET.get(toAdd.get(i));
                    Node n2 = NODESET.get(toAdd.get(j));
                    n1.connected.add(n2);
                    n2.connected.add(n1);
                }
            }
